...
bs=1000
retrieved_results_path=blip_queries/retrieved.jsonl
caption_count=$(wc -l < $retrieved_results_path)
echo $caption_count

for params in "1 fewshot_rag" "5 fewshot_rag" "0 zeroshot"; do
    set -- $params
    k=$1
    echo $k
    prompt_mode=$2
    ...
    for ((i = 0; i < $caption_count; i += $bs)); do
        echo $image_count
        next_index=$((i + bs))
        CUDA_VISIBLE_DEVICES=2 python text_to_image_inference.py \
